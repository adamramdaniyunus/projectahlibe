            ...
            <div className={"flex justify-center items-center h-full"}>
                <div onClick={handleClickInsideModal} className={'w-[500px] modal-shadow bg-white flex justify-center p-4 rounded-lg'}>
                    <div className={'flex flex-col gap-2 w-full'}>
                        <h1 className={'text-xl font-semibold text-gray-600'}>Delete post?</h1>
                        <div className="flex gap-2 justify-end">
                            <button className="px-4 py-2 bg-red-700 text-white rounded-lg disabled:bg-gray-500" onClick={handleDeletePost} disabled={loading}>{loading ? "Loading.." : "Yes"}</button>
                            <button className="px-4 py-2 bg-primary text-white rounded-lg" onClick={handleModal}>Cancel</button>
                        </div>
                    </div>
                </div>
            </div>
